 ...
 */
export function matchCron(cron, date) {
    ...
    return (matchPart(minute, m) &&
        matchPart(hour, h) &&
        matchPart(dom, d) &&
        matchPart(month, mon) &&
        matchPart(dow, w));
}
function matchPart(pattern, val) {
    if (pattern === "*")
        return true;
    if (pattern.includes(",")) {
 ...
 */
export function initScheduler(fastify) {
 ...
 */
async function executeScheduledWorkflow(workflow, triggerNodeId, fastify) {
    ...
    try {
        ...
        for (const node of orderedNodes) {
            ...
            if (node.type === "databaseNode" || node.type === "database") {
                ...
                if (!queryTemplate)
                    throw new Error(`Database node '${node.id}' missing query`);
            ...
            if (node.type === "customCodeNode" || node.type === "code") {
                ...
                if (!scriptCode)
                    throw new Error(`Code node '${node.id}' missing JS code`);
                const runResult = runInSandbox(scriptCode, executionContext);
                if (!runResult.success)
                    throw new Error(`Script Error in '${node.id}': ${runResult.error}`);
            ...
            if (node.type === "ifElseNode" || node.type === "ifelse") {
                ...
                if (!runResult.success)
                    throw new Error(`Condition Error in '${node.id}': ${runResult.error}`);
            ...
            if (node.type === "switchCaseNode" || node.type === "switchcase" || node.type === "switchCase") {
                ...
                if (!runResult.success)
                    throw new Error(`Switch Error in '${node.id}': ${runResult.error}`);
            ...
            if (node.type === "transformNode" || node.type === "transform") {
                ...
                if (!runResult.success)
                    throw new Error(`Transform Error in '${node.id}': ${runResult.error}`);
                executionContext.steps[node.id] = runResult.data;
                continue;
            }
            // JWT Validate Node
