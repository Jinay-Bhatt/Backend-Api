...
export async function createService(request, reply) {
    const { projectId } = request.params;
    const { name, baseUrl, description } = request.body;
    const user = request.user;
    if (!name || !baseUrl)
...
export async function updateService(request, reply) {
    ...
    const { name, baseUrl, description, isActive } = request.body;
    const user = request.user;
    try {
        const isOwner = await verifyProjectOwnership(projectId, user.id);
        if (!isOwner)
...
export async function createServiceRoute(request, reply) {
    const { projectId, serviceId } = request.params;
    const { path, method, targetService, description } = request.body;
    const user = request.user;
    if (!path || !method || !targetService)
        return reply.status(400).send({ error: "path, method, targetService are required" });
    try {
        const isOwner = await verifyProjectOwnership(projectId, user.id);
        if (!isOwner)
            return reply.status(403).send({ error: "Forbidden" });
...
export async function deleteServiceRoute(request, reply) {
...
export async function getGatewayConfig(request, reply) {
...
export async function upsertGatewayConfig(request, reply) {
    const { workflowId } = request.params;
    const { requireJwt, requireApiKey, apiKeyValue, rateLimitLimit, rateLimitWindow, corsEnabled, allowedOrigins } = request.body;
    const user = request.user;
    try {
