...
export async function getAnalytics(request, reply) {
    ...
    const { workflowId, range } = request.query;
    ...
    try {
        ...
        if (workflowId)
            logsWhere.workflowId = workflowId;
        ...
        const hourMap = new Map(); // hour -> count
...
export async function getLogs(request, reply) {
    const { projectId } = request.params;
    const { workflowId, limit = "50", page = "1", status } = request.query;
    const user = request.user;
    try {
        const project = await prisma.project.findFirst({
            where: { id: projectId, ownerId: user.id },
        ...
        const where = { workflow: { projectId } };
        if (workflowId)
