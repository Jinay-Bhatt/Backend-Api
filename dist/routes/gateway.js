        ...
        try {
            ...
            for (const node of orderedNodes) {
                ...
                if (node.type === "ifElseNode" || node.type === "ifelse") {
                    ...
                    if (!runResult.success) {
                        throw new Error(`Condition Evaluation Error in node '${node.id}': ${runResult.error}`);
                    }
                ...
                if (node.type === "switchCaseNode" || node.type === "switchcase" || node.type === "switchCase") {
                    ...
                    if (!runResult.success) {
                        throw new Error(`Switch Expression Evaluation Error in node '${node.id}': ${runResult.error}`);
                    }
                    const matchedValue = String(runResult.data);
                    executionContext.steps[node.id] = runResult.data;
                    // Check if matchedValue matches any case (ignoring 'default' in direct check)
                    const hasMatch = cases.filter((c) => c !== "default").includes(matchedValue);
                ...
                if (node.type === "httpClientNode" || node.type === "httpclient" || node.type === "httpClient") {
                    ...
                    const interpolatedUrl = urlTemplate.replace(/\$([a-zA-Z0-9_\.]+)/g, (match, path) => {
                    ...
                    if (headersTemplate) {
                        try {
                            const interpolatedHeaders = headersTemplate.replace(/\$([a-zA-Z0-9_\.]+)/g, (match, path) => {
                                const val = resolveVariable(path, executionContext);
                                return val !== undefined ? String(val) : "";
                            });
                    ...
                    if (bodyTemplate) {
                        if (typeof bodyTemplate === "string") {
                            if (bodyTemplate.startsWith("$") && !bodyTemplate.includes(" ")) {
                                ...
                                requestBody = bodyTemplate.replace(/\$([a-zA-Z0-9_\.]+)/g, (match, path) => {
                                    const val = resolveVariable(path, executionContext);
                                    return val !== undefined ? String(val) : "";
                                });
                            }
                    ...
                    try {
                        ...
                        if (["POST", "PUT", "PATCH", "DELETE"].includes(clientMethod.toUpperCase()) &&
                ...
                if (node.type === "transformNode" || node.type === "transform") {
                    ...
                    if (!runResult.success) {
                        throw new Error(`Data Transformation Error in node '${node.id}': ${runResult.error}`);
                    }
                    executionContext.steps[node.id] = runResult.data;
                    continue;
                }
                ...
                if (node.type === "jwtValidateNode" || node.type === "jwtValidate") {
                    ...
                    try {
                        const decoded = fastify.jwt.verify(token, { key: decryptedSecret });
                ...
                if (node.type === "apiKeyNode" || node.type === "apiKey") {
                    ...
                    const clientKey = request.headers[headerName];
                    if (!clientKey) {
                ...
                if (node.type === "responseNode" || node.type === "response") {
                    ...
                    if (responseHeadersTemplate) {
                        try {
                            const interpolatedHeaders = responseHeadersTemplate.replace(/\$([a-zA-Z0-9_\.]+)/g, (match, path) => {
                                const val = resolveVariable(path, executionContext);
                                return val !== undefined ? String(val) : "";
                            });
                    ...
                    if (redirectUrlTemplate) {
                        if (typeof redirectUrlTemplate === "string") {
                            if (redirectUrlTemplate.startsWith("$") && !redirectUrlTemplate.includes(" ")) {
                                ...
                                redirectUrl = redirectUrlTemplate.replace(/\$([a-zA-Z0-9_\.]+)/g, (match, path) => {
                                    const val = resolveVariable(path, executionContext);
                                    return val !== undefined ? String(val) : "";
                                });
                            }
